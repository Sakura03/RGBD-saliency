    ...
    def __call__(self, image, depth, label):
        if random.random() < 0.5:
            ...
            if x1 == 0:
                ...
                depth = cv2.flip(depth, 1) # veritcal flip
...
class RandomCrop(object):
    ...
    def __call__(self, image, depth, label):
        ...
        depth = cv2.resize(depth[i:i+size, j:j+size], (orig_w, orig_h), interpolation=cv2.INTER_LINEAR)
...
class RandomRotate(object):
    ...
    def __call__(self, image, depthm, label):
        ...
        depth_rotated = cv2.resize(depth_rotated, (orig_w, orig_h), interpolation=cv2.INTER_LINEAR)
        label_rotated = cv2.resize(label_rotated, (orig_w, orig_h), interpolation=cv2.INTER_NEAREST)

        return [image_rotated, depth_rotated, label_rotated]
...
class ToTensor(object):
    ...
    def __call__(self, image, depth, label):
        ...
        return [image_tensor, depth_tensor, label_tensor]
...
class ColorJitter(object):
    ...
    def __call__(self, image, depth, label):
        ...
        return [transform(image), depth, label]
