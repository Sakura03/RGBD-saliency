...
class BiSalNet(nn.Module):
    def __init__(self):
        ...
        self.rgb_path = VGG16_bn(pretrained=True)
        self.depth_path = VGG16_bn(pretrained=True)
        # self.global_context = PyramidPooling(512, 256)
        self.prepare_rgb = nn.ModuleList([
        ...
        self.prepare_depth = nn.ModuleList([
                convbnrelu(in_channel=512, out_channel=256, k=1, s=1, p=0, relu=False),
                convbnrelu(in_channel=512, out_channel=256, k=1, s=1, p=0, relu=False),
                convbnrelu(in_channel=256, out_channel=128, k=1, s=1, p=0, relu=False),
                convbnrelu(in_channel=128, out_channel=64, k=1, s=1, p=0, relu=False),
    ...
    def forward(self, x, depth): # (3, 1)
        size = x.size()[2:]
        rgb_stage1, rgb_stage2, rgb_stage3, rgb_stage4, rgb_stage5 = self.rgb_path(x)
        #(64, 1)    (128, 1/2)  (256, 1/4)  (512, 1/8)  (512, 1/16)
        # rgb_stage6 = self.global_context(rgb_stage5)                              # (256, 1/16)

        depth_stage1, depth_stage2, depth_stage3, depth_stage4, depth_stage5 = self.depth_path(depth)
        # (64, 1)      (128, 1/2)    (256, 1/4)    (512, 1/8)    (512, 1/16)

        fused_stage1 = self.fuse[0](self.prepare_rgb[0](rgb_stage5) + self.prepare_depth[0](depth_stage5))               # (256, 1/16)
        refined1 = interpolate(fused_stage1, rgb_stage4.size()[2:])                                                      # (256, 1/8)
        
        fused_stage2 = self.fuse[1](self.prepare_rgb[1](rgb_stage4) + self.prepare_depth[1](depth_stage4) + refined1)    # (128, 1/8)
        refined2 = interpolate(fused_stage2, rgb_stage3.size()[2:]) 		                                         # (128, 1/4)
        
        fused_stage3 = self.fuse[2](self.prepare_rgb[2](rgb_stage3) + self.prepare_depth[2](depth_stage3) + refined2)    # (64, 1/4)
        refined3 = interpolate(fused_stage3, rgb_stage2.size()[2:]) 		                                         # (64, 1/2)
        
        fused_stage4 = self.fuse[3](self.prepare_rgb[3](rgb_stage2) + self.prepare_depth[3](depth_stage2) + refined3)    # (32, 1/2)
        refined4 = interpolate(fused_stage4, rgb_stage1.size()[2:])		                                         # (32, 1)
        
        fused_stage5 = self.fuse[4](self.prepare_rgb[4](rgb_stage1) + self.prepare_depth[4](depth_stage1) + refined4)    # (16, 1)
